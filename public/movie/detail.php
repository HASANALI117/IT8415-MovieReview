...
<head>
    ...
    <style>
        ...
        .hero-banner { height: 400px; background: url('/<?php echo htmlspecialchars($movie['fanart_bg']); ?>') center/cover; position: relative; display: flex; align-items: flex-end; }
...
<div class="user-nav">
    <?php if (is_logged_in()): ?>
        Welcome, <strong><?php echo htmlspecialchars(current_username()); ?></strong>
        <a href="/auth/logout.php">Logout</a>
    <?php else: ?>
        <a href="/auth/login.php">Login to Review</a>
    <?php endif; ?>
</div>

<div class="hero-banner">
    <div class="hero-overlay">
        <h1 style="font-size: 3em; margin: 0;"><?php echo htmlspecialchars($movie['title']); ?></h1>
        <p style="margin: 5px 0;">
            <?php foreach($categories as $cat) echo "<span class='ui-state-default ui-corner-all' style='padding: 2px 8px; font-size: 0.8em; margin-right: 5px;'>".htmlspecialchars($cat['name'])."</span>"; ?>
        </p>
        <p style="font-size: 1.2em; opacity: 0.9;"><?php echo (int)$movie['release_year']; ?> • Rating: <?php echo htmlspecialchars($movie['avg_rating']); ?>/5</p>
    </div>
</div>

<div class="movie-container">
    <img src="/<?php echo htmlspecialchars($movie['image_url']); ?>" class="poster" alt="Poster"
         onerror="this.style.visibility='hidden';">
    <div class="details-card">
        <p><strong>Director:</strong> <?php echo htmlspecialchars($director_name); ?></p>
        <p style="line-height: 1.6; color: #444;"><?php echo nl2br(htmlspecialchars($movie['synopsis'])); ?></p>
        ...
        <?php else: ?>
            <p><em>Please <a href="/auth/login.php" style="color:#007bff">login</a> to rate this movie.</em></p>
        <?php endif; ?>
    </div>
</div>
